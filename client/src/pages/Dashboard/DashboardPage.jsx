...
function mkLine(yFmt) {
  return {
    ...
    plugins: {
      legend: { display: true, position: 'top', labels: { color: '#94a3b8', font: { family: 'Inter', size: 12 }, usePointStyle: true, padding: 16 } },
...
function mkBar(o = {}) {
  return {
    responsive: true, maintainAspectRatio: false, animation: { duration: 700 }, indexAxis: o.horizontal ? 'y' : 'x',
    plugins: { legend: { display: false }, tooltip: { backgroundColor: 'rgba(10,14,26,0.96)', padding: 14, cornerRadius: 10 } },
    scales: {
      x: { ticks: { color: '#64748b', font: { family: 'Inter', size: 11 }, callback: o.xFmt || (v => v), maxRotation: 30 }, grid: { color: 'rgba(148,163,184,0.05)' } },
...
function gradFill(color, alpha1 = 0.22, alpha2 = 0.01) {
  return ctx => {
    const ca = ctx.chart.chartArea; if (!ca) return `rgba(${color},${alpha1})`;
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    const loadExternalData = async () => {
      ...
      try {
        ...
        if (res1?.MailCount || res1?.SmsCount || res1?.WhatsappCount || res1?.RcsCount) {
          let count = res1.MailCount + res1.SmsCount + res1.WhatsappCount + res1.RcsCount;
          sessionStorage.setItem(
            'overallcount',
            count
          );
        }
        if (res2?.totalDaysToClose[0].totalDaysToClose) {
  ...
  const dashCards = [
    ...
    { l: 'Marketing Spend', v: `$${(dashKpis.marketing_spend || Number(sessionStorage.getItem('overallcount')) || 0).toLocaleString()}`, i: '💸' },
